...
class Kinematics(Node):
    ...
    def __init__(self):
        ...
        # 电脑（服务器）的 IP 地址和端口
        self.server_address = ('192.168.1.3', 12345)  # 使用你电脑的 IP 地址
        ...
        self.aim_angles = np.array([0.0,0.0,0.0])
    ...
    def calc_I_from_T(self,tor):
        return max(min((tor / self.k_cur_torque),self.max_ctrl_cur),-self.max_ctrl_cur)

    def calculate_angle(self,x, y):
        angle_radians = math.atan2(y, x) 
        angle_degrees = math.degrees(angle_radians) 
        # return angle_degrees
        print("degree",angle_degrees)
        # return max(min((((angle_degrees - 45) / 90) * 6.28 - 3.14),3.14),-3.14)
        return max(min((angle_degrees / 20.0),3.14),-3.14)
    ...
    def joint_state_callback(self, joint_pos_msg):
        ...
        vel1 = np.radians(joint_angles[3]) * self.frequency
        vel2 = np.radians(joint_angles[4]) * self.frequency
        vel3 = np.radians(joint_angles[5]) * self.frequency

        acc1 = vel1 - self.Pre_vel1
        acc2 = vel2 - self.Pre_vel2
        acc3 = vel3 - self.Pre_vel3
        ...
        # m = self.get_M_matrix([th1,th2,th3])
        m = self.get_M_matrix_new([th1,th2,th3])
        # c = self.get_C_matrix([th1,th2,th3],[vel1,vel2,vel3])
        c = self.get_C_matrix_new([th1,th2,th3],[vel1,vel2,vel3])
        ...
        N = np.eye(3) - J_inv @ J
        xy_angle = math.atan2(current_position[1], current_position[0]) 

        try:
            # 发送消息
            message = f"{np.round(np.linalg.norm(current_position),3)},{np.round(self.calculate_angle(current_position[0],current_position[1]),4)}"
            # message = f"{0.0},{0.0}"
            # message = "Jiaheng"
            print("send:", message)
        ...
        # u1_out = self.calc_I_from_T(T[0]) + self.apply_friction_compensation(vel1,self.calc_I_from_T(T[0]),self.friction_offset) 
        # u2_out = self.calc_I_from_T(T[1]) + self.apply_friction_compensation(vel2,self.calc_I_from_T(T[1]),self.friction_offset2)
        # u3_out = self.calc_I_from_T(T[2]) + self.apply_friction_compensation(vel3,self.calc_I_from_T(T[2]),self.friction_offset3)
        # u1_out = self.calc_I_from_T(-T[0][2]) + self.apply_friction_compensation(vel1,vel1,self.friction_offset) 
        # u2_out = self.calc_I_from_T(-T[1][2]) + self.apply_friction_compensation(vel2,vel2,self.friction_offset2) 
        # u3_out = self.calc_I_from_T(-T[2][2]) + self.apply_friction_compensation(vel3,vel3,self.friction_offset3) 
        ...
        joint_pos_rel_msg.data = [0.0,0.0,0.0]  # 转换为列表以便兼容
        ...
        # self.get_logger().info('Current position: {}\n err : {}\n T : {}\n input{}'.format(current_position,[self.Int_vel_err1,self.Int_vel_err2,self.Int_vel_err3],T,[u1_out,u2_out,u3_out]))
        # self.get_logger().info('Current position: {}, input{}'.format(current_position,[u1_out,u2_out,u3_out]))[self.Int_vel_err1,self.Int_vel_err2,self.Int_vel_err3]
    ...
    def calc_T_from_dynamics(self,p,d,dd,m,c):
        T = np.dot(m,dd) + c
        return T
...
def main(args=None):
    ...
    kinematics_node.client_socket.close()
